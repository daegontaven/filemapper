...
def create(path=None):
    """
    Automatically generates variables for files and maps them to their file paths.
    Call create() everytime a new file is added to update the maps.
    """
    if path != None:
        core.map_files(path)
    else:
...
def load(path=None, mode='r'):
    """
    Opens all the files in the root of a directory. If create() was called before calling load(),
    then it will be called again to override any previous maps.
    ...
    if err_files != []:
        raise IOError(', '.join(str(err_file) for err_file in err_files )+ " files are missing")
    global flag
    flag = True
...
def close(*args):
    """
    Calling this function without arguments closes all or any remaining files to be closed.
    Passing file names as arguments will close those files as needed. This function will not unmap
    any of the files mapped using the create function.
    """
    global closed_files
    if flag:
        if args == (): #This part closes all files
            ...
            if sys.version_info < (3,):
                difference_files = [i for i in [two_tuple[0] for two_tuple in file_dict.items()] if i not in closed_files]
            else:
                difference_files = [i for i in [two_tuple[0] for two_tuple in list(file_dict.items())] if i not in closed_files]
            for file_name in difference_files:
                file_dict[file_name].close() #This is all that's actually happening
            ...
            for file_name in args:
                if file_name not in closed_files:
                    if file_name in [two_tuple[0] for two_tuple in file_dict.items()]:
                        file_dict[file_name].close() #This is all that's actually happening
                        crr_files.append(file_name)
                        closed_files.append(file_name)
                    else:
            ...
            if err_files != []:
                raise IOError(', '.join(str(err_file) for err_file in err_files )+ " not defined in file_dict ")
            if same_files != []:
                if len(same_files) == 1:
                    raise IOError(', '.join(str(same_file) for same_file in same_files )+ " is closed already")
                else:
                    raise IOError(', '.join(str(same_file) for same_file in same_files )+ " were closed already")
            return tuple(crr_files)
    else:
...
def read(file_name):
    """
    Should be primarily used for accessing files mapped using the create() function.
    Note: The load() function automatically calls create() and also opens the files.
